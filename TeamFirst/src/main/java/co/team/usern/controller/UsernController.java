...
public class UsernController {
	...
	public String appuserNList(@RequestParam(value = "page", defaultValue = "1") Integer page,
		...
		return "app/Usern/appuserNList";
	}

	// 단건 + 조회수
	@GetMapping("/appUserh")
	public String appUserh(Model model, @RequestParam("board_no") int board_no) {
		model.addAttribute("us", usernService.getUserh(board_no));
		usernService.userHit(board_no);
		return "app/Usern/appUserh";
	}
